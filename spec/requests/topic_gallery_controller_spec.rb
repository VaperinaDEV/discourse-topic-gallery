...
describe "TopicGalleryController" do
  ...
  describe "GET /topic-gallery/:topic_id" do
    ...
    context "with filters" do
      ...
      it "returns all images for nonexistent username" do
        get "/topic-gallery/#{topic.id}.json", params: { username: "nonexistent_user_xyz" }

        expect(response.parsed_body["total"]).to eq(2)
      end

      it "filters by post_number" do
        get "/topic-gallery/#{topic.id}.json", params: { post_number: 2 }

        json = response.parsed_body
        expect(json["total"]).to eq(1)
        expect(json["images"].first["postNumber"]).to eq(2)
      end
      ...
      context "with date filters" do
        ...
        it "ignores invalid date strings" do
          get "/topic-gallery/#{topic.id}.json", params: { from_date: "not-a-date" }
    ...
    context "with combined visibility scenarios" do
      it "does not leak hidden post images through username filter" do
        post2.update!(hidden: true)
        sign_in(user)
      ...
      it "does not leak deleted post images through post_number filter" do
        post2.update!(deleted_at: Time.zone.now)
        sign_in(user)
      ...
      it "does not leak whisper images through date filter" do
        post2.update!(post_type: Post.types[:whisper])
        sign_in(user)

        get "/topic-gallery/#{topic.id}.json", params: { from_date: 1.year.ago.to_date.to_s }
